...
% stego_dir = sprintf(stego_dir_tpl, payload, QF);
...
%     steganography = params.steganography;
    ...
    for index = 1 : 5000
        cover_set{end + 1} = sprintf('%s/%d.jpg', cover_dir, training_set(index));
        stego_set{end + 1} = sprintf('%s/%d.jpg', stego_dir, training_set(index));
    end
    for index = 1 : 5000
        cover_set{end + 1} = sprintf('%s/%d.jpg', cover_dir, test_set(index));
        stego_set{end + 1} = sprintf('%s/%d.jpg', stego_dir, test_set(index));
    end

    fprintf('Extract GFR features ---- \n');
    
    gfr_features_path = sprintf('/data/lml/jpeg_test/gfr_cover%s.mat', num2str(params.listNum));
    if params.start == 0
        cover_features = extract_gfr(cover_set, QF);
    
        save(gfr_features_path, 'cover_features', '-v7.3');
    else
        features_mat = load(gfr_features_path);
        cover_features = features_mat.cover_features;
    end
%     cover_features = extract_gfr(cover_dir, QF);
        ...
        save_str = sprintf('juni-%.1f-jpeg-%d-ln-%d-sn-%d', payload, QF,params.listNum, params.ss_number);
        fprintf('GFR and ensemble results ----- \n')
        fprintf('Test accuracy for # %s #: %.4f \n', save_str, test_acc);

        file_id = fopen('acc_log_jpeg.txt','a');
        fprintf(file_id,'GFR__%s: %.4f\n', save_str, test_acc);
        fclose(file_id);
...
function dctr_features = extract_gfr(image_set, QF)
    extract_start = tic;
    file_num = 10000;
    dctr_features = zeros(file_num, 17000);
 
    parfor i=1:file_num
        jpeg_path = image_set{i};
        %j_struct = jpeg_read(jpeg_path);
       
        %j_f = DCTR(j_struct, QF);
        j_f = GFR(jpeg_path, 32, QF);

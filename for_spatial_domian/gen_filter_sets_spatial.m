function flag = gen_filter_sets_spatial(params)
    ...
    parfor index = 1:20000
        ...
        coverPath = [BB_Dir, '/', num2str(index), '.pgm'];
        ...
        f_array = zeros(3, w, w);
        f_array(1,:,:) = padarray(base_f, [floor(w / 2), 0]);
        f_array(2,:,:) = padarray(base_f', [0, floor(w / 2)]);
        f_array(3,:,:) = conv2(base_f, base_f') / -1;
